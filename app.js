...
// Generated voices sound more natural at normal speed.
// We slow them slightly during playback instead of regenerating audio.
// 1.00 = original generated audio. 1.00 = original human recording speed. Use 0.95 only if the recording feels too fast.
const AUDIO_PLAYBACK_RATE = 1.00;

// Pause between audio lines in Conversation, Missing words and Role-play partner audio.
// Increase this if the dialogue feels too fast between turns.
// 650 = natural classroom pause. Try 800 or 1000 if pupils need more time.
...
function playAudioOrFallback(situation, line, token, onEnd) {
  ...
  if (!source) {
    // No browser voice fallback for model conversations or role-play lines.
    // Word Bank still uses the browser voice for single words.
    const pause = Math.min(4200, Math.max(1400, String(line.text || "").length * 80));
    activeAudioTimer = setTimeout(onEnd, pause);
    return;
  }
  ...
  // Keep the original pitch when the browser supports it.
...
function showSelectedVoice() {
  ...
  if (voice) {
    console.log(`Speaking Friends voice: ${voice.name} (${voice.lang})`);
  } else {
    console.log("Speaking Friends voice: browser default");
  }
}
...
function getConversationLines(situation, mode = "conversation") {
  // Main learning path uses one stable model conversation.
  // This keeps the teacher-recorded audio, Conversation, Missing words and Role-play aligned.
...
async function shareOptionalChallengeAudio(situationId) {
  ...
  const text = `
Speaking Friends optional challenge audio
...
function openOptionalChallengeGmailDraft(situationId) {
  ...
  const student = optionalChallengeStudentInfo(situationId);
  const dialogue = optionalChallengeFinalLines(situation).join("\n");
  const subject = `Speaking Friends optional challenge - ${student.name} - ${student.group} - ${situation.title}`;

  const body = `
Hello,

Here is my Speaking Friends optional challenge audio.
